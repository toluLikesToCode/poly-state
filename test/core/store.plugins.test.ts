...
describe("PluginManager", () => {
  ...
  describe("onError", () => {
    it("should call onError on all plugins without using executeHook", () => {
      ...
      expect(plugin1.onError).toHaveBeenCalledWith(
        testError,
        context,
        mockStore
      );
    ...
    it("should prevent infinite recursion by not calling handleError", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "[PluginManager] Plugin failing-plugin.onError failed:"
        ),
  ...
  describe("beforeStateChange", () => {
    it("should transform action through plugins", () => {
      ...
      const result = pluginManager.beforeStateChange(
        action,
        testState,
        mockStore
      );

      expect(plugin1.beforeStateChange).toHaveBeenCalledWith(
        action,
        testState,
        mockStore
      );
  ...
  describe("onStateChange", () => {
    it("should notify all plugins of state change", () => {
      ...
      expect(plugin1.onStateChange).toHaveBeenCalledWith(
        newState,
        testState,
        action,
        mockStore
      );
      expect(plugin2.onStateChange).toHaveBeenCalledWith(
        newState,
        testState,
        action,
        mockStore
    ...
    it("should handle null action", () => {
      ...
      expect(plugin.onStateChange).toHaveBeenCalledWith(
        newState,
        testState,
        null,
        mockStore
  ...
  describe("beforePersist", () => {
    it("should transform state before persistence", () => {
      ...
      const result = pluginManager.beforePersist(
        testState,
        StorageType.Local,
        mockStore
      );
      ...
      expect(plugin1.beforePersist).toHaveBeenCalledWith(
        testState,
        StorageType.Local,
        mockStore
      );
  ...
  describe("onPersisted", () => {
    it("should notify all plugins after persistence", () => {
      ...
      expect(plugin1.onPersisted).toHaveBeenCalledWith(
        testState,
        StorageType.Session,
        mockStore
      );
      expect(plugin2.onPersisted).toHaveBeenCalledWith(
        testState,
        StorageType.Session,
        mockStore
      );
  ...
  describe("onStateLoaded", () => {
    it("should transform loaded state", () => {
      ...
      const result = pluginManager.onStateLoaded(
        partialState,
        StorageType.Local,
        mockStore
      );
  ...
  describe("onCrossTabSync", () => {
    it("should transform synced state", () => {
      ...
      const result = pluginManager.onCrossTabSync(
        syncedState,
        "session123",
        mockStore
      );
  ...
  describe("Batch Operations", () => {
    ...
    describe("onBatchEnd", () => {
      it("should notify all plugins when batch ends", () => {
        ...
        const actions: ActionPayload<TestState>[] = [
          { count: 1 },
          { count: 2 },
        ];
        ...
        expect(plugin1.onBatchEnd).toHaveBeenCalledWith(
          actions,
          finalState,
          mockStore
        );
        expect(plugin2.onBatchEnd).toHaveBeenCalledWith(
          actions,
          finalState,
          mockStore
        );
  ...
  describe("Transaction Operations", () => {
    ...
    describe("onTransactionEnd", () => {
      it("should notify all plugins when transaction succeeds", () => {
        ...
        expect(plugin1.onTransactionEnd).toHaveBeenCalledWith(
          true,
          mockStore,
          changes,
          undefined
        );
        expect(plugin2.onTransactionEnd).toHaveBeenCalledWith(
          true,
          mockStore,
          changes,
          undefined
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle plugins with hooks that return undefined", () => {
      ...
      expect(plugin.beforeStateChange).toHaveBeenCalledWith(
        action,
        testState,
        mockStore
      );
