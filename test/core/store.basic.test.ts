 ...
 *
 * Comprehensive test suite for the Universal Store's core functionality including:
...
describe("Path-based Updates (updatePath)", () => {
  ...
  describe("Basic Path Updates", () => {
    ...
    it("should support function-based updates with current value", () => {
      store.updatePath(
        ["profile", "address", "zip"],
        (currentZip: number) => currentZip + 1
      );
  ...
  describe("Auto-creation of Missing Paths", () => {
    it("should create missing intermediate objects", () => {
      ...
      expect(store.getState().metadata?.created).toEqual(
        new Date("2024-01-01")
      );
    });
  ...
  describe("Complex Path Update Scenarios", () => {
    it("should handle array manipulation", () => {
      store.updatePath(["tags"], (currentTags: string[]) => [
        ...currentTags,
        "react",
      ]);
    ...
    it("should work with plugin integration", () => {
      ...
      const [newState, prevState, actionApplied] =
        mockOnStateChange.mock.calls[0];
